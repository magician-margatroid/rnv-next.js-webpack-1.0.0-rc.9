...
const AppThemed = () => {
    const destinationNameUrl = toDestinationNameUrl({ name: 'foobar' })
    ...
    return (
        <View style={theme.styles.container}>
            ...
            <Text style={theme.styles.textH2} {...testProps('template-starter-home-screen-version-number-text')}>
                Destination {destinationNameUrl}
            </Text>
            <Text style={theme.styles.textH3}>
                {`platform: ${Api.platform}, factor: ${Api.formFactor}, engine: ${Api.engine}`}
            </Text>
            <Text style={theme.styles.textH3}>{`hermes: ${typeof HermesInternal === 'object' && HermesInternal !== null ? 'yes' : 'no'
                }`}</Text>
